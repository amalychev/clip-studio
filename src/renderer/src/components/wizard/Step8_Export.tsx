...
export function Step8_Export() {
  ...
  const timelineImages = timelineImageIds.length
    ? timelineImageIds.map(id => images.find(img => img.id === id)).filter(Boolean) as typeof images
    : images
  ...
  const handleExport = async () => {
    ...
    const payload = {
      ...
      image_filenames: timelineImages.map(i => i.filename),
        ...
        <div className="grid grid-cols-2 gap-3">
          {exportFormats.map((fmt: VideoFormat) => (
            <Card
              ...
              hover
              active={fmt.enabled}
              onClick={() => { if (!generating) { toggleFormat(fmt.id); setExported(false) } }}
